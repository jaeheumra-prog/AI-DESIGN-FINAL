...
const PORTFOLIO_URL =
  ...
  (process.env.NODE_ENV === "development"
    ? "http://127.0.0.1:4175/shorts.html"
    : "https://ai-design-2-pi.vercel.app/shorts.html");
...
type EntryMode = "role" | "jobseeker-survey" | "recruiter";
...
function formatFieldSummary(fields: string[], fallback: string) {
  return fields.length > 0 ? fields.join(", ") : fallback;
}

function createShortsUrl(targetFile: "shorts1.html" | "shorts2.html") {
  const portfolioUrl = new URL(PORTFOLIO_URL, window.location.href);
  portfolioUrl.pathname = portfolioUrl.pathname.replace(/[^/]+$/, targetFile);
  return portfolioUrl;
}
...
function CandidateSurveyScreen({
  ...
  const steps = [
    ...
    },
    {
      ...
      caption: "검색해서 찾고, 지금 관심 있는 분야를 여러 개 골라둘 수 있습니다.",
      ...
      content: (
        <SelectablePills
          ...
          selected={survey.field}
          onToggle={(value) =>
            setSurvey((current) => ({
              ...current,
              field: current.field.includes(value)
                ? current.field.filter((field) => field !== value)
                : [...current.field, value],
            }))
          }
          multi
...
function CandidateProfilePreview({
  ...
  const fieldSummary = formatFieldSummary(survey.field, "Aspiring Creative Talent");
  const heroImage =
    survey.field.some(
      (field) => field.includes("Design") || field.includes("Illustration") || field.includes("VFX"),
    )
  ...
  const projectCards = [
    {
      title: `${survey.field[0] || "Creative"} Proof Reel`,
          ...
          <div className="space-y-8">
            <div className="space-y-5">
              ...
              <h1 className="text-4xl font-medium tracking-tight sm:text-5xl lg:text-6xl">
                {draftName}
                <span className="mt-2 block text-[#7b6b5d]">{fieldSummary}</span>
              </h1>
          ...
          <div className="rounded-[1.75rem] border border-[#e6ddd1] bg-white p-7 shadow-[0_20px_50px_rgba(50,34,18,0.06)]">
            ...
            <div className="mt-6 space-y-5 text-sm leading-7 text-[#4f463e]">
              <div>
                ...
                <p className="mt-1 text-base font-medium text-[#1f1b17]">{formatFieldSummary(survey.field, "미선택")}</p>
              </div>
              <div>
...
export default function OasisStage() {
  ...
  const compactFieldSummary = formatFieldSummary(survey.field.slice(0, 2), "새로운 분야");
  ...
  const goToShortsPortfolio = (targetFile: "shorts1.html" | "shorts2.html") => {
    const portfolioUrl = createShortsUrl(targetFile);
    portfolioUrl.searchParams.set("from", "oasis");
    portfolioUrl.searchParams.set("oasis", window.location.origin);
    window.location.href = portfolioUrl.toString();
  };
  ...
  const handleProfileEnter = () => {
    ...
    setNotice(
      realtimeMode === "supabase"
        ? `${draftName}님, ${compactFieldSummary} 프로필이 라운지에 연결됐어요. 이제 다른 컴퓨터에서도 당신의 좌석이 보이기 시작합니다.`
        : `${draftName}님, ${compactFieldSummary} 프로필이 준비됐어요. 지금은 로컬 데모 모드라 같은 브라우저 안에서만 좌석이 공유됩니다.`,
    );

    goToShortsPortfolio("shorts1.html");
  };
      ...
      {!showLibraryUi ? (
        entryMode === "role" ? (
          ...
          <RecruiterScreen
            ...
            onSearch={() => goToShortsPortfolio("shorts2.html")}
          />
        )
      ) : null}
